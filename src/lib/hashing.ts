// Web Crypto hashes + a small MD5 fallback for legacy comparison.
export type HashAlgo = "SHA-256" | "SHA-384" | "SHA-512" | "MD5";
...
/* Minimal MD5 (public domain, Joseph Myers). Included only for legacy comparison. */
export function md5(str: string): string {
  ...
  function cb(s: string) {
    ...
    const numBlocks = ((n + 8) >> 6) + 1;
    const blocks = new Array(numBlocks * 16).fill(0);
    for (let i = 0; i < n; i++) blocks[i >> 2] |= bytes[i] << ((i % 4) * 8);
    blocks[n >> 2] |= 0x80 << ((n % 4) * 8);
    blocks[numBlocks * 16 - 2] = n * 8;
    return blocks;
  }
  const x = cb(str);
  let a = 1732584193,
