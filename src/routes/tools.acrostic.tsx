  ...
  head: () => ({
    meta: [
      { title: "Acrostic maker — Easter" },
      {
        name: "description",
        content:
          "Hide a word in the first letters of each line, or extract one from existing text.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        eyebrow="Acrostic"
        title="Hide a word in plain sight"
        description="Use the first letter of each line to reveal a secret word."
      />
      <Tabs defaultValue="make">
        <TabsList className="glass-panel mb-6 grid w-full grid-cols-2 rounded-xl p-1">
          <TabsTrigger value="make" className="rounded-lg">
            Make
          </TabsTrigger>
          <TabsTrigger value="extract" className="rounded-lg">
            Extract
          </TabsTrigger>
        </TabsList>
  ...
  return (
    <GlassCard className="space-y-5 p-6">
      <div className="space-y-2">
        <label className="text-sm font-medium">Hidden word</label>
        <Input
          value={word}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <CopyButton value={lines} />
        </div>
        <Textarea
          rows={Math.max(word.length, 4)}
          value={lines}
          readOnly
          className="bg-black/30 font-mono text-sm"
        />
        <p className="text-xs text-muted-foreground">
          Replace each <span className="font-mono">…</span> with a line that starts with the
          highlighted letter.
        </p>
      </div>
    </GlassCard>
    ...
    <GlassCard className="space-y-5 p-6">
      ...
      <div className="flex gap-2">
        {(["line", "sentence"] as const).map((m) => (
          <Button
            ...
            onClick={() => setMode(m)}
            className={
              mode === m ? "bg-primary text-primary-foreground" : "border-white/10 bg-white/5"
            }
          >
      ...
      <div className="space-y-2">
        ...
        <div className="break-all rounded-lg border border-white/10 bg-black/30 p-3 font-mono text-sm">
          {result || <span className="text-muted-foreground">Paste text above…</span>}
        </div>
      </div>
    </GlassCard>
  );
