...
export const Route = createFileRoute("/tools/css-hidden-text")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Generate playful CSS/HTML snippets for hidden text reveals.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        eyebrow="Hidden CSS"
        title="Tiny in-page reveals"
        description="Playful HTML/CSS snippets that hide text in plain sight."
      />
      <GlassCard className="space-y-5 p-6">
        <div className="space-y-2">
          <label className="text-sm font-medium">Hidden text</label>
        ...
        <p className="text-xs text-muted-foreground">
          This is not secure. It's a playful easter egg.
        </p>
      </GlassCard>
    </div>
  );
...
function build(msg: string, mode: Mode) {
  ...
  if (mode === "hover")
    return `<span style="color:transparent;transition:color .3s;" onmouseover="this.style.color='currentColor'" onmouseout="this.style.color='transparent'">${safe}</span>`;
