  ...
  return (
    <div className="mx-auto max-w-4xl">
      <PageHeader
        eyebrow="Reveal Lab"
        title="Easter Hunter"
        description="Paste text or drop a file. Easter looks for hints, encodings and possible hidden clues. Detections are guesses."
      />
  ...
  const run = () => {
    ...
    if (f.length === 0)
      f.push({
        ...
        message: "No common patterns detected. Try the encoder lab or the puzzle reverse path.",
      });
    setFindings(f);
  };
  const clear = () => {
...
function FileHunt() {
  ...
  const handle = async (f: File) => {
    ...
    out.push({
      ...
      message: sig
        ? `Looks like ${sig.ext} (${sig.mime}).`
        : "No common signature recognised in the first bytes.",
      to: "/tools/file-signature",
      value: Array.from(buf.slice(0, 16))
    ...
    if (strs.length)
      out.push({
        ...
        message: "Found readable text fragments inside the file.",
        value: strs.join("\n"),
        to: "/tools/strings",
      });
    detected.forEach((d) =>
      out.push({ title: d.label, message: d.note, to: routeFor(d.key), ctaLabel: ctaFor(d.key) }),
    );
    setFindings(out);
  };
  return (
    <div className="space-y-5">
      <GlassCard className="p-6">
        <FileDropzone
          ...
          onFile={handle}
          fileName={name}
          hint="Any local file. Processed in your browser."
        />
      </GlassCard>
      {findings.length > 0 && (
        <div className="grid gap-3 sm:grid-cols-2">
