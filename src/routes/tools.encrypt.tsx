...
export const Route = createFileRoute("/tools/encrypt")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Encrypt and decrypt a message locally with a password using AES-GCM.",
      },
    ],
  }),
  component: Page,
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        ...
        description="Encrypted with AES-GCM and a key derived from your password. Everything happens in your browser."
      />
      ...
      <Tabs defaultValue="encrypt">
        <TabsList className="glass-panel mb-6 grid w-full grid-cols-2 rounded-xl p-1">
          <TabsTrigger value="encrypt" className="rounded-lg">
            Encrypt
          </TabsTrigger>
          <TabsTrigger value="decrypt" className="rounded-lg">
            Decrypt
          </TabsTrigger>
        </TabsList>
  ...
  return (
    <GlassCard className="space-y-5 p-6">
      ...
      <Button
        onClick={async () => {
          try {
            setOut(await encryptText(msg, pw));
  ...
  return (
    <GlassCard className="space-y-5 p-6">
      ...
      <Button
        onClick={async () => {
          try {
            setOut(await decryptText(msg, pw));
      ...
      {out && (
        <div className="space-y-2">
          ...
          <div className="whitespace-pre-wrap break-words rounded-lg border border-white/10 bg-black/30 p-3 text-sm">
            {out}
          </div>
        </div>
      )}
