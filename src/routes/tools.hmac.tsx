  ...
  head: () => ({
    meta: [
      { title: "HMAC generator — Easter" },
      {
        name: "description",
        content:
          "Generate or verify an HMAC — a keyed hash that proves a message matches a secret key.",
      },
    ],
  }),
  component: Page,
...
function Page() {
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        eyebrow="HMAC"
        title="Keyed verification"
        description="An HMAC proves that a message matches a secret key — without revealing the key."
      />
  ...
  return (
    <GlassCard className="space-y-5 p-6">
      ...
      <Field label="Secret key">
        <Input value={key} onChange={(e) => setKey(e.target.value)} className="bg-white/5" />
      </Field>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium">HMAC</label>
          <CopyButton value={out} />
        </div>
        <div className="break-all rounded-lg border border-white/10 bg-black/30 p-3 font-mono text-xs">
          {out || <span className="text-muted-foreground">Provide message and key…</span>}
        </div>
      </div>
    </GlassCard>
  );
