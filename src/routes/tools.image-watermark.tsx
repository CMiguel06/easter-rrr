...
function Page() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader
        eyebrow="Watermark"
        title="Add a visible watermark"
        description="A simple, subtle signature for your image. Processed locally."
      />
      <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <GlassCard className="space-y-5 p-6">
          <FileDropzone
            ...
            onFile={handleFile}
            fileName={name}
            hint="PNG, JPEG — anything your browser can render"
          />
          <div className="space-y-2">
            <label className="text-sm font-medium">Watermark text</label>
            <Input value={text} onChange={(e) => setText(e.target.value)} className="bg-white/5" />
          </div>
          <div className="space-y-2">
          ...
          {out ? (
            <div className="space-y-4 w-full">
              <img
                ...
                className="mx-auto max-h-[420px] rounded-xl border border-white/10"
              />
              <a
                href={out}
                download="easter-watermark.png"
            ...
            <img
              src={src}
              alt="Original"
              className="max-h-[420px] rounded-xl border border-white/10 opacity-70"
            />
          ) : (
