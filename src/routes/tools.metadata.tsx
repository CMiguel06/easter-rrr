...
export const Route = createFileRoute("/tools/metadata")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "See what your photos quietly reveal — and export a cleaned version.",
      },
      { property: "og:title", content: "Analyse file metadata — Easter" },
      {
        property: "og:description",
        content: "Inspect EXIF and other metadata fields. Files stay in your browser.",
      },
    ],
  }),
    ...
    try {
      ...
      if (data) {
        ...
        if (data.DateTimeOriginal)
          rows.push({
            key: "Taken on",
            value: new Date(data.DateTimeOriginal).toLocaleString(),
            sensitive: true,
          });
        if (data.ExifImageWidth && data.ExifImageHeight)
          rows.push({
            key: "Dimensions",
            value: `${data.ExifImageWidth} × ${data.ExifImageHeight}`,
          });
      ...
      setFields([
        ...
        { key: "Size", value: `${(f.size / 1024).toFixed(1)} KB` },
      ]);
    }
  };
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <GlassCard className="space-y-5 p-6">
        <FileDropzone
          ...
          onFile={onFile}
          fileName={file?.name}
          hint="Images: full EXIF read. Other files: basic info only."
        />
        {fields && (
          <div className="space-y-3">
            {hasSensitive && (
              <div className="flex items-start gap-2 rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-3 text-xs text-yellow-100/90">
                ...
                Some fields here can identify you, your device or your location. Consider stripping
                them before sharing.
              </div>
            )}
                ...
                <tbody>
                  {fields.map((f) => (
                    <tr key={f.key} className="border-b border-white/5 last:border-0">
                      <td className="w-1/3 bg-white/[0.02] px-4 py-2.5 text-muted-foreground">
                        {f.key}
                      </td>
                      <td className="px-4 py-2.5 font-mono text-xs break-all">
                        {f.value}
                        {f.sensitive && (
                          <span className="ml-2 rounded bg-yellow-500/20 px-1.5 py-0.5 text-[10px] text-yellow-100">
                            sensitive
                          </span>
                        )}
                      </td>
        ...
        <div className="rounded-lg border border-white/10 bg-white/[0.02] p-3 text-xs text-muted-foreground">
          What this means: metadata is information stored inside a file — like the camera that took
          a photo, when it was taken, and sometimes where. It travels with the file when you share
          it.
        </div>
      </GlassCard>
    </div>
