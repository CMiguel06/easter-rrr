...
function Page() {
  ...
  const save = async () => {
    ...
    a.href = url; a.download = "easter-note.json"; a.click();
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader eyebrow="Secret note" title="A small encrypted note file" description="Write a note, optionally encrypt it with a password, then save it as a local file. Re-open later with the same password." />
      <GlassCard className="space-y-5 p-6">
        <Textarea rows={8} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Write your note…" className="bg-white/5" />
        <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 p-3">
          <span className="text-sm">Encrypt with password</span>
          <Switch checked={encrypt} onCheckedChange={setEncrypt} />
        </div>
        {encrypt && <Input type="password" value={pw} onChange={(e) => setPw(e.target.value)} placeholder="Password" className="bg-white/5" />}
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={() => inputRef.current?.click()} variant="outline" className="border-white/10 bg-white/5">
